...
function densityFixture() {
  ...
  const lesson = (key: string, name: string, date: string, weekday: number, sections: number[], adjustment: any = null) => ({
    ...
    classroom: adjustment?.endpoint === 'proposed' ? proposed.room : original.room,
    classroom_short: 'B416-1', class_label: classes, student_count: 128,
...
async function mount(page: Page) {
  await page.route('http://schedule-density.test/**', async route => {
    ...
    await route.fulfill({ contentType: 'text/html', body: `<!doctype html><html lang="zh-CN"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><style>*{box-sizing:border-box}body{margin:16px;font-family:Arial,sans-serif}</style><div id="deck"></div><script type="module">
      ...
      window.deck.setOverview(${JSON.stringify(densityFixture())});window.deck.openExpanded();
    </script></html>` });
  });
...
async function readCard(card: Locator) {
  return card.evaluate(node => {
    ...
    const metadata = [...node.querySelectorAll<HTMLElement>('.cs-lesson__meta')];
    const visibleMetadata = metadata.filter(span => { const rect = span.getBoundingClientRect(); return rect.width > 0 && rect.height > 0 && getComputedStyle(span).visibility !== 'hidden'; });
    return { width: box.width, height: box.height, title: title.textContent, titleWidth: titleBox.width, titleHeight: title.offsetHeight, lineHeight: parseFloat(getComputedStyle(title).lineHeight),
      ...
      metadataCount: metadata.length, visibleMetadata: visibleMetadata.map(span => span.textContent),
...
function expectCompact(metrics: Awaited<ReturnType<typeof readCard>>, hasButton: boolean) {
  ...
  if (hasButton) {
    ...
    expect(Array.from(metrics.buttonText).length).toBeLessThanOrEqual(4);
    ...
    expect(metrics.titleButtonOverlap).toBe(false);
  }
}

test('crowded original, proposed, room, cancel and ordinary cards show readable titles with concise controls', async ({ page }, testInfo) => {
  await page.setViewportSize({ width: 1182, height: 650 });
  await mount(page);
  for (const key of ['move-old', 'move-new', 'room', 'cancel', 'ordinary']) expectCompact(await readCard(card(page, key)), key !== 'ordinary');
  ...
  for (const key of ['move-old', 'move-new', 'room', 'cancel', 'ordinary']) {
...
test('preview restores all lesson information and complete button wording on the same classroom link', async ({ page }, testInfo) => {
  ...
  expect(metrics.visibleMetadata.join(' ')).toContain('大成楼C108');
  ...
  expect(closing.visibleMetadata.join(' ')).toContain('大成楼C108');
  expect(closing.buttonText).toContain('正在申请变更');
...
test('slot-size changes select concise or detailed content without rebuilding data or losing controls', async ({ page }) => {
  await mount(page);
  for (const key of ['move-old', 'ordinary']) {
    ...
    expect(spacious.visibleMetadata.length).toBeGreaterThanOrEqual(3);
    expect(spacious.titleFits).toBe(true);
...
test('height boundaries and narrow overlap lanes reserve complete title lines and concise controls', async ({ page }) => {
  ...
  for (const key of ['move-old', 'move-new', 'room', 'cancel']) {
    ...
    for (const height of [58, 68, 69, 79, 80, 92, 80, 79, 69, 68]) {
      ...
      if (height >= 80 && key !== 'room') expect(metrics.titleHeight, `height ${height} can display the full two-line course title`).toBeGreaterThanOrEqual(2 * metrics.lineHeight - 1);
    }
