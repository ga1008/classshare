...
class ProcessMaterialWorkflowContractTests(unittest.TestCase):
    ...
    def test_exam_grade_manage_generation_stays_on_page_and_only_reveals_final_material(self):
        ...
        styles = Path("static/css/ui-system.src.css").read_text(encoding="utf-8")
        insight_macro = Path("templates/macros/manage_insights.html").read_text(encoding="utf-8")
        ...
        self.assertIn("document_type: 'exam_grade_record'", script)
        ...
        self.assertIn('data-insight-key="{{ data_key }}"', insight_macro)
        self.assertIn("updateInsightRing('materials-file-ratio'", script)
        self.assertIn("isFinalMaterialCatalogue ? '查看正文' : '文档'", script)
        self.assertIn("? '最终材料'", script)
        self.assertIn("item?.eligible === false", classroom_script)
    ...
    def test_final_grade_transcript_generation_syncs_roster_and_closes_source_loop(self):
        ...
        self.assertIn("/final-grade-transcript/source-candidates", script)
        ...
        self.assertIn("expected_roster_signature", script)
        self.assertIn("materials-final-grade-refresh-btn", template)
        self.assertIn("查看并核对全部", script)
        self.assertIn("final-grade-source-card__issues", script)
        self.assertIn("_create_generated_final_grade_transcript_material", helpers)
        self.assertIn("正在锁定已核对的名单缓存与两份成绩来源", script)
        self.assertIn("source?.generate_url", script)
        self.assertIn("data-materials-final-grade-manual-source", script)
        self.assertIn("function openManageFinalGradeSourcePicker", script)
        self.assertIn("context_mismatches", script)
        self.assertIn("await revealRecentlyGeneratedMaterial(materialId)", script)
        self.assertIn("sync_classroom_exam_roster_from_academic_system", router)
        self.assertIn("ACADEMIC_EXAM_ROSTER_CACHE_SECONDS", router)
        self.assertIn("min_refresh_interval_seconds=ACADEMIC_EXAM_ROSTER_CACHE_SECONDS", router)
        self.assertIn("build_final_grade_transcript_readiness", router)
        self.assertIn("build_final_grade_transcript_payload", router)
        self.assertIn("list_final_grade_transcript_source_candidates", router)
        self.assertIn("ORDER BY row_order ASC, id ASC", service)
        self.assertIn("_same_context(fields, context)", service)
        self.assertIn("_record_matches_offering_scope", service)
        self.assertIn("_identity_text(source.get(\"student_name\"))", service)
        self.assertIn(".materials-final-grade-wizard", styles)
        self.assertIn("#materials-classroom-generate-submit-btn.is-loading:disabled", styles)
        self.assertIn("来源状态尚未判定", script)
        self.assertIn("generation.requestId === requestId", script)
    ...
    def test_process_material_editors_offer_pdf_export_actions(self):
        ...
        self.assertIn("async function applySignatureBinding(role, signatureIds)", assessment_script)
        self.assertIn("async function commitSignatureBinding(role, ids)", assessment_script)
        self.assertIn("setSaveState('is-saving', '更新签名中');", assessment_script)
        self.assertIn("onConfirm: (ids) => commitSignatureBinding(role, ids)", assessment_script)
    ...
    def test_material_detail_surfaces_ai_import_summary(self):
        ...
        self.assertIn("lessondoc-20260901", island)
