...
<title>bidisync Visual Sync Console</title>
  ...
  tailwind.config = {
    theme: {
      extend: {
        ...
        fontFamily: {
          sans: ['-apple-system','BlinkMacSystemFont','Segoe UI','Microsoft YaHei','Arial','sans-serif'],
...
<header class="sticky top-0 z-20 border-b border-slate-700/80 bg-slate-950/90 backdrop-blur">
  <div class="mx-auto flex max-w-7xl flex-col gap-4 px-5 py-5 sm:flex-row sm:items-center sm:justify-between lg:px-8">
    <div>
      <h1 class="text-xl font-bold tracking-tight text-white">bidisync Visual Sync Console</h1>
      <p class="mt-1 text-xs text-slate-400">Local project on the left, remote project on the right. Compares path, mtime, size, and SHA-256.</p>
    </div>
    <span class="inline-flex w-fit items-center rounded-full border border-slate-700 bg-slate-900 px-3 py-1.5 text-xs font-semibold text-slate-300">SSH password is used in memory only</span>
  </div>
</header>
<main class="mx-auto grid max-w-7xl gap-5 px-5 py-6 lg:px-8">
  <section class="overflow-hidden rounded-2xl border border-slate-700/80 bg-slate-900/90 shadow-2xl shadow-slate-950/30">
    <div class="border-b border-slate-700/80 px-5 py-4">
      <h2 class="text-sm font-bold text-white">Connection and Sync Settings</h2>
    </div>
    <div class="grid grid-cols-12 gap-4 px-5 py-5">
      <div class="col-span-12 sm:col-span-6 lg:col-span-3"><label class="mb-1 block text-xs text-slate-400">Remote IP</label><input id="remote_host" value="{{ remote_host }}" class="block w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-white focus:border-brand-500 focus:ring-brand-500"></div>
      <div class="col-span-6 lg:col-span-2"><label class="mb-1 block text-xs text-slate-400">Port</label><input id="remote_port" value="{{ remote_port }}" class="block w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-white focus:border-brand-500 focus:ring-brand-500"></div>
      <div class="col-span-6 lg:col-span-2"><label class="mb-1 block text-xs text-slate-400">User</label><input id="remote_user" value="{{ remote_user }}" class="block w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-white focus:border-brand-500 focus:ring-brand-500"></div>
      <div class="col-span-12 lg:col-span-5"><label class="mb-1 block text-xs text-slate-400">Remote project path</label><input id="remote_root" value="{{ remote_root }}" class="block w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-white focus:border-brand-500 focus:ring-brand-500"></div>

      <div class="col-span-12 sm:col-span-6 lg:col-span-4"><label class="mb-1 block text-xs text-slate-400">SSH password</label><input id="password" type="password" autocomplete="current-password" placeholder="Enter the server password" class="block w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-white focus:border-brand-500 focus:ring-brand-500"></div>
      <div class="col-span-6 lg:col-span-2"><label class="mb-1 block text-xs text-slate-400">Direction filter</label><select id="only" class="block w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-white focus:border-brand-500 focus:ring-brand-500"><option value="all">All</option><option value="upload">Uploads only</option><option value="download">Downloads only</option></select></div>
      <div class="col-span-6 lg:col-span-2"><label class="mb-1 block text-xs text-slate-400">Transfer workers</label><input id="workers" type="number" min="1" max="32" value="{{ workers }}" class="block w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-white focus:border-brand-500 focus:ring-brand-500"></div>
      <div class="col-span-12 lg:col-span-4"><label class="mb-1 block text-xs text-slate-400">Options</label><div class="flex flex-wrap items-center gap-4 rounded-xl border border-slate-700 bg-slate-950/70 px-3 py-2.5 text-sm text-slate-300"><label class="inline-flex items-center gap-2"><input id="no_delete" type="checkbox" class="rounded border-slate-600 bg-slate-900 text-brand-600 focus:ring-brand-500">Do not propagate deletes</label><label class="inline-flex items-center gap-2"><input id="docker_enabled" type="checkbox" checked class="rounded border-slate-600 bg-slate-900 text-brand-600 focus:ring-brand-500">Use Docker Compose</label></div></div>

      <div class="col-span-12 lg:col-span-4"><label class="mb-1 block text-xs text-slate-400">Docker Compose directory</label><input id="docker_project_dir" value="{{ docker_project_dir }}" class="block w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-white focus:border-brand-500 focus:ring-brand-500"></div>
      <div class="col-span-12 sm:col-span-6 lg:col-span-4"><label class="mb-1 block text-xs text-slate-400">down command</label><input id="docker_down_cmd" value="{{ docker_down_cmd }}" class="block w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-white focus:border-brand-500 focus:ring-brand-500"></div>
      <div class="col-span-12 sm:col-span-6 lg:col-span-4"><label class="mb-1 block text-xs text-slate-400">up command</label><input id="docker_up_cmd" value="{{ docker_up_cmd }}" class="block w-full rounded-xl border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-white focus:border-brand-500 focus:ring-brand-500"></div>

      <div class="col-span-12 flex flex-wrap items-center gap-3 pt-1">
        <button class="rounded-xl bg-slate-700 px-4 py-2.5 text-sm font-bold text-white hover:bg-slate-600 disabled:cursor-not-allowed disabled:opacity-50" id="testBtn">Test SSH</button>
        <button class="rounded-xl bg-brand-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-brand-700 disabled:cursor-not-allowed disabled:opacity-50" id="scanBtn">Scan Diff</button>
        <button class="rounded-xl bg-emerald-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-emerald-700 disabled:cursor-not-allowed disabled:opacity-50" id="syncBtn" disabled>Sync Selected</button>
        <button class="rounded-xl bg-slate-700 px-4 py-2.5 text-sm font-bold text-white hover:bg-slate-600 disabled:cursor-not-allowed disabled:opacity-50" id="selectAllBtn" disabled>Select All</button>
        <button class="rounded-xl bg-slate-700 px-4 py-2.5 text-sm font-bold text-white hover:bg-slate-600 disabled:cursor-not-allowed disabled:opacity-50" id="selectNoneBtn" disabled>Select None</button>
        <span class="text-sm text-slate-400" id="status">Waiting for scan</span>
      </div>
    </div>
    <div class="flex flex-wrap gap-2 border-t border-slate-700/80 px-5 py-4" id="stats"></div>
  </section>

  <section class="overflow-hidden rounded-2xl border border-slate-700/80 bg-slate-900/90 shadow-2xl shadow-slate-950/30">
    <div class="border-b border-slate-700/80 px-5 py-4"><h2 class="text-sm font-bold text-white">Diff Comparison</h2></div>
    <div class="flex flex-col gap-3 border-b border-slate-700/80 bg-slate-950/50 px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
      <input id="filter" placeholder="Filter paths, for example templates, py, or db" class="block w-full max-w-md rounded-xl border border-slate-700 bg-slate-950 px-3 py-2.5 text-sm text-white focus:border-brand-500 focus:ring-brand-500">
      <div class="text-sm text-slate-400" id="selectedInfo">Nothing selected</div>
    </div>
    <div class="hidden grid-cols-12 border-b border-slate-700/80 bg-slate-900 px-5 py-3 text-xs font-semibold uppercase tracking-wide text-slate-400 lg:grid"><div class="col-span-5">Local</div><div class="col-span-1 text-center">Action</div><div class="col-span-4">Remote</div><div class="col-span-2">Status</div></div>
    <div class="max-h-[60vh] overflow-auto" id="diffList"></div>
  </section>

  <section class="overflow-hidden rounded-2xl border border-slate-700/80 bg-slate-900/90 shadow-2xl shadow-slate-950/30">
    <div class="border-b border-slate-700/80 px-5 py-4"><h2 class="text-sm font-bold text-white">Sync Progress</h2></div>
    <div class="grid gap-3 px-5 py-5">
      <div class="h-3 overflow-hidden rounded-full border border-slate-700 bg-slate-950"><div class="h-full w-0 rounded-full bg-gradient-to-r from-brand-600 to-emerald-400 transition-all" id="bar"></div></div>
      <div class="text-sm text-slate-400" id="progressText">Not started</div>
      <div class="h-56 overflow-auto rounded-xl border border-slate-800 bg-black p-4 font-mono text-xs leading-6 text-slate-300" id="log"></div>
    </div>
  </section>
</main>
<script>
let scanId=null,actions=[],selected=new Set(),currentJob=null;const $=id=>document.getElementById(id);
const pillClass='inline-flex items-center gap-1 rounded-full border border-slate-700 bg-slate-950/80 px-3 py-1.5 text-xs text-slate-300';
const rowClass='grid grid-cols-12 items-center gap-3 border-b border-slate-800 px-5 py-3 hover:bg-brand-500/10';
const cellClass='min-w-0 text-sm text-slate-200';
const metaClass='rounded-lg border border-slate-700 px-2 py-1 text-xs font-semibold text-slate-300';
...
function renderStats(summary,dockerPlan){const c=summary.counts||{};$('stats').innerHTML=[`<span class="${pillClass}">Total <b class="text-white">${summary.total}</b></span>`,`<span class="${pillClass}">Upload <b class="text-white">${c.upload||0}</b></span>`,`<span class="${pillClass}">Download <b class="text-white">${c.download||0}</b></span>`,`<span class="${pillClass}">Conflict <b class="text-white">${c.conflict_skip||0}</b></span>`,`<span class="${pillClass}">Transfer <b class="text-white">${summary.bytes_human}</b></span>`,`<span class="${pillClass}">DB uploads <b class="text-white">${summary.db_upload_count}</b></span>`,`<span class="${pillClass}">Docker <b class="text-white">${dockerPlan}</b></span>`].join('');}
function actionColor(kind){return kind==='upload'?'text-emerald-300':(kind==='download'?'text-sky-300':(kind==='conflict_skip'?'text-amber-300':'text-slate-300'));}
function renderActions(){const q=$('filter').value.trim().toLowerCase(),box=$('diffList');let html='',shown=0;for(const a of actions){if(q&&!a.rel.toLowerCase().includes(q))continue;shown++;const checked=selected.has(a.id)?'checked':'';const indent=a.depth*18;const lm=a.local?`${esc(a.local.mtime)} - ${esc(a.bytes_human||'')}`:'Missing';const rm=a.remote?`${esc(a.remote.mtime)} - ${esc(a.bytes_human||'')}`:'Missing';html+=`<div class="${rowClass}" title="${esc(a.reason)}"><div class="${cellClass} col-span-12 lg:col-span-5"><label class="flex min-w-0 items-center gap-2"><input type="checkbox" data-id="${esc(a.id)}" ${checked} class="rounded border-slate-600 bg-slate-900 text-brand-600 focus:ring-brand-500"><span class="truncate font-mono" style="padding-left:${indent}px">${esc(a.rel)}</span></label><div class="mt-1 truncate text-xs text-slate-500">${lm}</div></div><div class="col-span-2 text-center text-lg font-black lg:col-span-1 ${actionColor(a.kind)}">${esc(a.arrow)}</div><div class="${cellClass} col-span-10 lg:col-span-4"><div class="truncate font-mono" style="padding-left:${indent}px">${esc(a.rel)}</div><div class="mt-1 truncate text-xs text-slate-500">${rm}</div></div><div class="hidden min-w-0 lg:col-span-2 lg:block"><span class="${metaClass}">${esc(a.kind)}</span> ${a.is_db_upload?'<span class="ml-2 text-xs font-bold text-amber-300">DB</span>':''}<div class="mt-1 truncate text-xs text-slate-500">${esc(a.reason)}</div></div></div>`}box.innerHTML=html||'<div class="px-5 py-8 text-sm text-slate-400">No matching diffs.</div>';box.querySelectorAll('input[type=checkbox]').forEach(cb=>cb.onchange=()=>{cb.checked?selected.add(cb.dataset.id):selected.delete(cb.dataset.id);updateSelectedInfo();});updateSelectedInfo(shown);}
function updateSelectedInfo(shown){$('selectedInfo').textContent=`Selected ${selected.size} item(s)${shown!==undefined?', showing '+shown+' item(s)':''}`;$('syncBtn').disabled=!scanId||selected.size===0;$('selectAllBtn').disabled=!scanId;$('selectNoneBtn').disabled=!scanId;}
...
$('testBtn').onclick=async()=>{if(!$('password').value){alert('Enter the SSH password first');return}setBusy(true,'Testing SSH connection...');$('log').textContent='';try{const r=await fetch('/api/test-ssh',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload())});const data=await r.json();if(!r.ok||!data.ok)throw new Error(data.error||data.message||'SSH connection test failed');$('status').textContent='SSH connection OK';$('log').textContent=`SSH connection OK\nBanner: ${data.banner||''}`;}catch(e){$('status').textContent='SSH connection failed';$('log').textContent=e.message;alert(e.message)}finally{setBusy(false,$('status').textContent)}};
$('scanBtn').onclick=async()=>{if(!$('password').value){alert('Enter the SSH password first');return}setBusy(true,'Scanning local and remote...');$('log').textContent='';try{const r=await fetch('/api/scan',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload())});const data=await r.json();if(!r.ok)throw new Error(data.error||'Scan failed');scanId=data.scan_id;actions=data.actions;selected=new Set(actions.filter(a=>a.default_selected).map(a=>a.id));renderStats(data.summary,data.docker_plan);renderActions();$('status').textContent=`Scan completed: ${actions.length} diff(s)`}catch(e){$('status').textContent='Scan failed';alert(e.message)}finally{setBusy(false,$('status').textContent)}};
$('syncBtn').onclick=async()=>{if(!scanId){alert('Scan first');return}if(!$('password').value){alert('Enter the SSH password first');return}if(selected.size===0){alert('No sync item selected');return}if(!confirm(`Sync ${selected.size} selected item(s)?`))return;setBusy(true,'Creating sync job...');$('bar').style.width='0%';$('progressText').textContent='Job submitted';$('log').textContent='';try{const body=payload();body.scan_id=scanId;body.selected_ids=[...selected];const r=await fetch('/api/start-sync',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const data=await r.json();if(!r.ok)throw new Error(data.error||'Start failed');currentJob=data.job_id;pollJob()}catch(e){alert(e.message);setBusy(false,'Sync start failed')}};
async function pollJob(){if(!currentJob)return;const r=await fetch('/api/job/'+currentJob);const j=await r.json();const pct=Math.min(100,Math.round((j.done_bytes||0)/(j.total_bytes||1)*100));$('bar').style.width=pct+'%';$('progressText').textContent=`Status: ${j.status}; files ${j.done_items||0}/${j.total_items||0}; failed ${j.failed_items||0}; transfer ${pct}%`+(j.error?`; ${j.error}`:'');$('log').textContent=(j.logs||[]).join('\n');$('log').scrollTop=$('log').scrollHeight;if(j.status==='running'||j.status==='queued')setTimeout(pollJob,1000);else setBusy(false,j.status==='done'?'Sync completed':'Sync failed')}
</script>
