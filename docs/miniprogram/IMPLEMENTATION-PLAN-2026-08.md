...
- [ ] **签到（学生）**：`/api/mp/classroom` 聚合进行中的签到 → 一键签到（复用 `smart_attendance_entry_service` 通道）；签到结果即时反馈。
- [ ] **签到（教师）**：发起/结束签到、实时到课统计投影。
- [ ] **投票参与（学生）**：进行中投票列表 + 投票页（单选/多选/可见时机遵守既有规则，复用 `/api/polls` bearer 直通，mp 只做列表聚合）。
- [ ] **投票发起（教师）最小集**：从已有草稿投票一键"开始/结束"（创建编辑留在 Web）。
- [ ] **课堂互动最小集（学生）**：教师发起的课堂答题在小程序作答（复用 `/api/classroom-interactions`；弹幕/大屏观感留在 Web）。
- [ ] 课堂 tab 装配：进行中事项（签到/投票/互动）置顶 + 我的课程列表。
- [ ] 技术决策点：实时性优先用**轮询**（5–10s，课堂 tab 在前台才轮询），不上 WebSocket——2c/4GB 服务器与小程序 wx.connectSocket 复杂度都不划算，200 并发量级轮询可承受。
- [ ] 出口标准：一个真实课堂"教师发起签到→学生小程序签到→教师看到统计"全程真机跑通；投票投出并计票正确。

### M3 学习闭环补全（版本 v0.13.x）——低成本高感知

- [ ] **成绩视图**：`/api/mp` 投影 `student_report_card_service`（学期成绩概览 + 单课明细只读）。
- [ ] **错题本**：错题列表 + 详情只读投影（`student_wrong_book_service`）；从任务结果页可跳转。
- [ ] **修为/积分**：我的页顶部个人卡（修为值/连续打卡/积分余额，复用 achievements/points 服务投影；积分商城兑换留在 Web，只展示）。
- [ ] **"我的"页成型**：按 §4 结构装配功能列表。
- [ ] 出口标准：学生从"交作业→看分→看错题→看成绩单"在小程序内闭环。
