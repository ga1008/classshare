...
**一、常驻面板的玻璃是预渲染的贴图，不是实时模糊。**（2026-09-23 改）

页面背景层是 `position: fixed` 的固定图，且只取自随程序发布的图库。同一张图、同一种模糊，每帧重算毫无意义——而 `backdrop-filter` 正是每帧重算。等到「所有板块都是玻璃」之后，首页背着**六个 16px 宿主铺满 88% 视口**，在真实 GPU 上就是一场重绘风暴（现场症状：开屏乱闪、首页显示异常）。

于是模糊移到构建期：`tools/tips/compress_images.py` 为每张图库图产出一份 `frost/<同名>.webp`（宽 48px，浏览器放大时的插值就是模糊，单张约 300 字节）。面板用 `background-attachment: fixed` + `cover` 直接画它身后那一块——这两个值让该层按**视口**定位，与背景层对齐，滚动时也不脱节。

配方只写一次，在 `materials.css` 的 `@media (hover: hover) and (pointer: fine) and (prefers-reduced-transparency: no-preference) and (forced-colors: none)` 里，三层自上而下：材质自己的填充 → 复现背景层不透明度的暗面纱 → 预模糊图；底下垫页面基色。**页面级遗留面板的清单也住在这条规则里**——让每个页面模块各写一遍，正是 37 处模糊声明散落到 8 个文件的由来。加一个边界 = 加一行选择器。

三个必须记住的坑：

- **`--lq-material-fill` 对页面级面板是未定义的**，配方里必须写 `var(--lq-material-fill, var(--lq-page-content))`。否则整条 `background-image` 在计算值阶段失效、塌回 `none`，而填充仍由别的声明落上——面板看起来是完成的，霜层只是**悄悄不见了**。
- **选择器前缀 `:root:not([data-lq-glass="off"]):not([data-lq-tier="C"]):not([data-lq-contrast="more"])` 不只是语义。** 这些面板同时命中的遗留页面规则用的是 `background` 简写，会把 `background-image` 一并重置；`.ls-page.role-student .ls-tools` 三个类就足以压过配方，于是一块面板有霜层、隔壁那块没有。把「什么时候不该有霜」写进选择器，既说明了意图，又让模块稳定地站在页面 CSS 之上。
- **无障碍条件写在媒体查询里，不写成后面的取消规则。** 配方的 `:is()` 带两个属性选择器，取消规则只带一个——降低透明度的用户会输掉这场特异性比较、照样吃到霜层。一条压根不生效的规则没法被压过。

**实时模糊只留给短暂浮层**：对话框、灯箱、toast、tooltip。它们盖的是页面内容而不是背景图，静态贴图在那里是错的；而且同时至多一两个。验收把视口内宿主**封顶 3**（外壳顶栏 + 窄屏 Dock + 至多一个浮层），实测 12 个场景多数为 0、最高 2。

嵌套守卫保留：材质套材质只贡献自己的填充，不重画基色——重画会用不透明色盖掉父层的霜。控件级永不做宿主。
...
## 材质模块与统一交互（2026-09-23，S9）

用户反馈两点：背景图只占一小块；其他板块出不来毛玻璃。两者同源——背景图被 `contain` 塞在一条 46vh 的带里，16:9 的图按高度缩放后只剩视口一半宽，玻璃背后没有东西可透。

### 背景层

`inset: 0` + `background-size: cover`。`cover` 同样保持原图比例，只是裁掉溢出。面纱同步铺满，上重下轻。

### 采用式材质 API

任意板块、按钮、下拉写一个属性即获得完整配方：

```html
<div data-lq-material="content">…</div>
```

五个取值对应 §12 的材质标度（`clear` / `control` / `chrome` / `content` / `raised`），配方含填充、边界模糊、高光边、圆角、描边、投影。**尺寸与色彩仍由各块自己决定**，通过 `--lq-material-fill` / `--lq-material-radius` / `--lq-material-shadow` 覆盖。

高光边用 `inset box-shadow` 而不是 `::before`：遗留块常已占用伪元素，再加一个会打架。

### 统一交互

```html
<button data-lq-interactive>…</button>
```

统一过渡（时长与缓动取自令牌）、悬停微抬与发光、按下、聚焦环。**位移量放在自定义属性 `--lq-lift` / `--lq-press` 里**，减少动效偏好只需改变量：若写成直接覆盖 `transform`，后来任何人给悬停选择器加一个 `:not()` 就会在特异度上压过取消规则，动效对声明了偏好的用户重新出现。组件包实际踩到过这个坑。

### 三条不变量

1. **填充与模糊是两个独立决定。** 每种材质都半透明，但只有边界模糊。把两者写进同一条规则，会导致"把某个选择器移出模糊清单"时连它的半透明也一起拿掉——本轮实际发生过。
