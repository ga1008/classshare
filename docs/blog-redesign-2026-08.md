...
### P4 阅读体验增强 ✅ 2026-08-19（进度条/时长/prose 排版/骨架屏/精选区/TOC 全部落地）
- [x] 详情页文章目录：宽屏右侧悬浮 h2/h3 目录 + 滚动高亮 + 平滑跳转（≥2 标题才显示）；阅读顶栏吸顶偏移 94px 避开全站导航栏。
...
### P5 收尾 ✅ 2026-08-19（已两次部署上线：release 20260819-135233 / 20260819-140338；远端与容器内 blog-cyberdeck.css 已 ssh 清理，线上 404 确认）
- [x] 响应式两档已截图验证（1440/390），768 档由 CSS 断点覆盖（375/768/1280）+ 无障碍点检（tab 焦点环、aria 不回退）。
- [x] 暗色模式：已核实全站当前**没有**暗色主题机制（无 .dark 类体系），此项 N/A；待全站引入暗色令牌后博客自动跟随。
