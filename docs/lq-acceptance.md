...
## S9 续：背景图可见度（2026-09-23）

用户反馈背景图"非常模糊、根本看不清"，前面板块的磨砂因此也出不来。

### 根因不是模糊，是三重削弱叠加

背景层同时被压了三次：饱和度 45%、不透明度 55%、再盖一层覆盖全屏的面纱。图库本身是 1536×1024，在 1440 宽下是轻微缩小而非放大，**图片本身并不虚**。

### 处置与各自的依据

| 参数 | 原值 | 新值 | 依据 |
|---|---|---|---|
| 饱和度 | .45 / .35 | **.95 / .85** | 对比度是亮度计算，色度几乎不参与，所以颜色可以全部还回来，且不动任何对比度余量 |
| 不透明度 | .55 / .42 | **.58 / .47** | 这是对比度保证允许的上限，由 `test_lq_tokens` 的材质模型跨六调色板解出 |
| 面纱 | .38 / .50 | **.12 / .22** | 保证本身是在**不含面纱**的前提下算的，面纱是纯余量，因此可以让出去 |
| 面板模糊 | thin (8px) | **regular (16px)** | 图能看清了，磨砂才有东西可磨；细半径压在一张灰糊的图上等于没有 |

提高不透明度到上限后，13 个语义前景在新边界下不再达标，已按同一套解法只调明度收紧。守卫里的假设值同步更新为实际发布值——否则那条保证描述的就不是线上的东西。
...
| `tools/test_backend.py`（隔离全量） | Ran 3792 tests, OK (skipped=205) |
| `npx vitest run` | 78 files / 672 tests passed |
| `tsc --noEmit` / `lint_lq.py` | 通过 / blocking 为空 |

过程中页面用例一度 11 项失败，断言是"期望暗色、实际浅色"。追下去是**我自己的临时探针把夹具账号的外观偏好显式写成了 light**，而用例靠模拟系统配色切换、前提是该偏好为 `auto`。把账号重置回 `auto` 后 12 项全过。不是回退，但值得记：**共享夹具账号的偏好是跨用例状态，临时探针改了要还原。**
