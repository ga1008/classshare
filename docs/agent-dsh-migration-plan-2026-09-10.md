...
最新生产状态：官方 DSH 已替换旧 TUI 上线，发布号 `20260910-215713-b420b6c46e55`，旧容器退役、专属状态可逆隔离。原生迁移、三身份隔离任务和生产运行验收详见 `agent-dsh-final-gates-2026-09-10.md`。以下保留各阶段历史记录；尚未接入的业务与禁用的子工作流仍以能力台账为准，不因本次上线而宣称全部完成。
...
### 14.4 本人确认、账户与签章并发和子工作流验证

上述批次已固定为本地检查点 `0283be34`。后续将本人业务确认拆分为成绩、签章和教学生命周期领域：签章使用申请可以由实际有权审批的教师、学生或管理员本人核对文档后批准/拒绝；空班级、未引用课程和合班通过真实影响快照、手工名称、说明与逐项警告确认。旧确认遇到材料、名单、权限或引用变化失效，业务及声明回执同事务回滚。新增的 C 领域确认身份锁明确与撤权、退出登录串行化；5 项原生 PostgreSQL 对应比赛通过。31 项相关后端合并和11项真实Chrome组件通过，详细边界与命令见 `agent-human-business-confirmation-2026-09-10.md`。教学普通写入引用保护和整个新检查点仍需最终合并验收。

账户、材料、签章、流程、申请的共享锁协议完成新一轮审查，168 项相关验证通过，其中28项为真实 PostgreSQL。归并签章时，仍由当前导出材料活跃引用的源签章会明确阻断，不以软删除使原材料静默丢章；不可变历史原文不被重写。证据在 `agent-signature-decisions-poc-2026-09-10.json`，签章认领/归并等仍不因此直接变为 Agent 可执行能力。

子工作流已有单层、同任务跨尝试累计最多4次的服务端准入原型，固定 DSH 上真实执行了两个子任务（含结构化结果）及一次取消。实际发现 ACP 取消并销毁本地子会话后，远端 MCP HTTP 工作可能仍在执行；服务端 finish 只记运行器观察，不退还累计额度，也不冒充宿主证实已停止。父子工具过滤和本地并发限制不构成同容器恶意运行器的硬隔离。因此 profile 继续禁用子代理/工作流，不能宣称所有插件已激活。证据见 `agent-dsh-subagent-enablement-2026-09-10.md` 和 `agent-dsh-bounded-workflow-validation-2026-09-10.json`。新 profile/插件进入后续冻结镜像后，还须重新验证镜像摘要和完整任务，不能沿用早期镜像验收。
