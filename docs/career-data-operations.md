...
`tl` 的每个阶段仍为 `[阶段, 职位名称, 职责说明]`。第二项必须是该节点对应的完整职位 title，图中标签、悬浮提示、键盘朗读和详情共用此字段。阶段描述职责发展的参考顺序，横轴同时显示该列以毕业为起点的大概用时。

`career_stage_service.build_career_stages` 统一处理内置方向、专业基础网络和历史网络。已维护方向使用对应的岗位序列，未知方向兼容历史完整职位；旧的“了解与观察”等活动文字不再作为职位显示。新 AI compact 候选通过 `role_titles` 提供四个完整职位，平台校验后写入原有 `tl`，阶段说明仍由平台生成。公共视图版本升级会使浏览器结果版本失效；读取时修复展示，不改写历史网络、方向标识、收藏或跨方向连线，也不触发额外 AI 生成。

毕业年限与共享时间轴
------------------

`career_stage_service.build_career_time_axis()` 返回平台维护的 `time_axis`：`basis="graduation"`、说明 `label/note`，以及四个 `columns`。每列包含 `stage`（0–3）、`phase`、`years_min`、`years_max` 和完整显示 `label`；最后一列的 `years_max=null` 表示开放区间。
...
四列使用全图统一口径，切换方向和跨方向连线不会把同一横坐标换成另一套年限。毕业是第 0 年；年限不是每一阶段分别再花多久。上下时间轴、节点悬浮提示与详情共用此契约，学生的预计毕业时间用于补充距现在的换算，不覆盖毕业后的参考区间。若已毕业，当前位置按毕业后的时间定位，避免把“现在”错误画在毕业之前；毕业时间未知时保留毕业后参考年限。

列距按时间段排布，属于分段参考轴，不是等比例年份尺。紫色连线保留原有的转向关系（含转入另一方向的起步岗位），不表示时间先后；不能沿跨方向连线累加或倒扣毕业年限。时间位置也不等同于学生已获得该职位。

这些区间是产品规划刻度，不是就业市场统计或到年自动晋升的保证；进修、转行及需要执业资格的路径，实际时间还取决于教育和资格条件。`time_axis.note` 在界面随时间信息展示。公共视图 `career-public-view-v3` 对新旧网络均重新生成此字段，不采用旧 AI 的阶段年限或薪资承诺。函数每次返回独立对象；公共投影保持幂等且不修改输入，不新增数据库字段、不回写历史网络、不因读取入队 AI 任务。
