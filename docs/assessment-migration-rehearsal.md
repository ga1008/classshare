...
`deployment/deploy_remote.ps1` 仍遵守原 `.gitignore`，是包含本机部署参数的本地操作脚本；本次没有取消忽略、force-add 或将其放进服务器包。隔离交付目录会显式复制它来运行 DryRun。可复用的原生报告校验器、跨平台门禁测试和本说明正常交付；PowerShell 本地脚本合同测试只在 Windows 且该本地脚本存在时执行，干净 checkout 不会因缺少它而失败。签入的源码和报告不会包含真实密钥或数据库备份。

需要受控停写迁移时，显式追加 `-QuiesceForMigration`，并提供本次原生报告和对应备份；该开关禁止同时使用 `-SkipDatabaseBackup`，服务器也必须实际配置 PostgreSQL。解压前核对正在运行的六个应用容器没有额外 `/app` 源码绑定，发现异常绑定就停止，避免旧进程读到新代码。当前 canonical compose 只绑定运行数据，允许先完成新镜像构建；旧镜像另外保留 `rollback-时间-服务` 标签，不受 dangling-image 清理影响。

镜像构建成功后，停止 app、ai、mailer、blog-crawler、scheduler、agent-worker，逐项确认已停止，保留 PostgreSQL。然后再次执行带 5 秒锁等待上限的 `pg_dump`，备份成功并通过 gzip 校验才执行新镜像的独立 `init_database()`。必须必需表齐全、无跳过步骤、无索引失败，才统一启动服务。停止、备份或迁移失败均不继续启动；该失败路径已用无 Docker／数据库／网络的隔离命令桩验证。部署者仍需在切换前确认无正在执行的考试批改、外部写入和代理运行任务，并完成运行配置及附件备份。

该开关不自动恢复旧数据库或旧容器。失败时保留停服状态及备份，先检查迁移结果再选择恢复流程；如果服务已启动并产生新提交，不能直接用切换前备份覆盖数据库。DryRun 即使带此开关也仅进行本机打包和报告校验，不执行任何远端操作。
