...
## 9. P4 历史双开课堂「合并向导」方案（2026-08-31 评审通过并按推荐实现）

> **P4.0 已上线**（3bc43d3d）：§9.7 四项均按推荐拍板（作业并存 / 入口全放开 /
> source 物理删除 / 仅教师本人）。实现要点与方案一致；两处实测修正：
> ① REBUILD_CACHE 类经唯一约束实测收紧为 REPOINT（更保守，零数据丢失）；
> ② 聚合缓存表（classroom_behavior_states / emoji_usage_stats / custom_emojis /
> smart_attendance_student_advice）从 guarded 改为 DEDUP_SKIP——教师行在双开
> 课堂两边必然重复，不应阻断合并（成绩/进度/证书类仍保持冲突阻断）。
> 本地真实数据演练：动态web #6+#7 合并 11155 行 / 1.7s / 快照 7.5MB，
> 名单 74 人按班分节、3 份同名作业带「（原软工2401班）」后缀。
