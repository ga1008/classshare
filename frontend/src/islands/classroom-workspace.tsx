    ...
    <Dialog open={panel !== null} onOpenChange={value => { if (!value) close(); }}>
      <DialogContent className="cw-dialog classroom-page classroom-workspace-v2" onOpenAutoFocus={event => {
        ...
        <div className="cw-dialog-scroll">
          {displayPanel === 'tasks' && !tasks.some(task => taskMatchesFilter(task, teacher, filter, query)) && <p className="cw-empty" role="status">没有符合当前筛选条件的任务。</p>}
          {displayPanel === 'timeline' && !indexSessions.length && <p className="cw-empty" role="status">没有匹配的课次，请更换搜索内容。</p>}
          {displayPanel === 'timeline' ? <div className="cw-timeline-index">{indexSessions.map(item => <button type="button" key={String(item.order_index)} className="cw-timeline-index-item" data-cw-session-order={item.order_index} aria-pressed={String(item.order_index) === String(session?.order_index)} aria-haspopup="dialog" onClick={() => document.dispatchEvent(new CustomEvent('classroom:select-session', { detail: { order: item.order_index } }))}><span>{item.session_number_label}</span><strong>{item.segment_title || item.detail_title || item.title}</strong><small>{item.session_date || item.session_status_label}</small></button>)}</div> : displayPanel && <ExistingSurface panel={displayPanel} filter={filter} query={query} tasks={tasks} teacher={teacher} restoreScroll={restoreScroll} />}
        </div>
      </DialogContent>
    </Dialog>
  </>;
