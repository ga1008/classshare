...
if os.name != 'posix' or root != Path('/lanshare/.codex-temp/dsh-migration-20260910'):
...
def main():
    config = SimpleNamespace(image=args.image, profile=root / 'deployment/dsh/profile',
                             task_root=root / 'fixture-tasks', state_root=root / 'fixture-state',
                             socket_root=root / 'fixture-ipc', max_concurrency=1, max_runtime_seconds=120)
    for directory in (config.task_root, config.state_root, config.socket_root): directory.mkdir(exist_ok=True)
    ...
    request = {'version': 1, 'action': 'run', 'task_id': task_id, 'attempt_id': str(uuid.uuid4()),
               ...
               'tools_token': tools_token, 'model': 'deepseek-v4-pro', 'search_model': ''}
    ...
    try:
        ...
        for prompt in ('poc-mcp', 'poc-write', 'poc-shell', 'poc-question'):
...
try:
    ...
    (root / 'linux-poc-result.json').write_text(json.dumps(report, indent=2, ensure_ascii=False) + '\n')
