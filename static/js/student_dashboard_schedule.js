...
export function initStudentDashboardSchedule(root) {
  ...
  const renderCourses = () => {
    ...
    find('course-list').innerHTML = visible.length ? visible.map(course => `<article class="ls-schedule-course"><h3><a href="${courseURL(course)}">${escapeHtml(course.course_name)}</a></h3><p>${escapeHtml([course.class_name, course.teacher_name].filter(Boolean).join(' · '))}</p><small>${escapeHtml(course.semester || '未配置学期')}${course.is_unscheduled ? ' · 未排定' : course.unpositioned_session_count ? ` · ${Number(course.unpositioned_session_count)} 次课待排定` : ''}${course.is_history ? ' · 往期' : ''}</small><div><a class="ls-button ls-button-primary" href="${courseURL(course)}">进入课堂</a><a class="ls-link" href="${courseURL(course)}#assignment-panel">作业与考试</a></div></article>`).join('')
      : `<div class="ls-schedule-empty"><strong>${courses.length ? '没有匹配的课程' : '还没有加入课堂'}</strong><p>${courses.length ? '可清除筛选，查看全部学期的课程。' : '加入课堂后，课程和待办会出现在这里。'}</p>${courses.length ? '<button type="button" class="ls-button" data-student-course-reset>清除集合筛选</button>' : ''}</div>`;
  };
