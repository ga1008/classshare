...
export function createPopover(options) {
    ...
    panel.classList.add('twb-popover', `twb-popover--${kind}`);
    panel.hidden = true;
    ...
    function position() {
        ...
        panel.classList.toggle('twb-popover--sheet', sheet);
        if (sheet) {
            panel.style.left = '';
            panel.style.top = '';
            return;
        ...
        panel.classList.toggle('twb-popover--flipped', flipped);
    }
    ...
    function trapTab(event) {
        ...
        const focusables = Array.from(panel.querySelectorAll('button:not([disabled]), input:not([disabled]), [tabindex]:not([tabindex="-1"])'))
    ...
    api.open = () => {
        ...
        if (options.modal) {
            ...
            backdrop.className = 'twb-backdrop';
            backdrop.addEventListener('pointerdown', (event) => {
                if (event.target === backdrop) api.close('backdrop');
            });
            layer.appendChild(backdrop);
            panel.classList.add('twb-popover--modal');
        }
...
export const POPOVER_TIMING = { OPEN_MS, CLOSE_MS };
