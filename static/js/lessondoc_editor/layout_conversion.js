...
export function convertLayout(source,layout,measurements={}) {
    ...
    slide.layout=layout;
    if(layout==='canvas') {
        slide.objects=[...flow.map((b,i)=>({...b,frame:b.frame||measurements[b.id]||{x:80+(i%2)*570,y:120+Math.floor(i/2)*150,w:540,h:130}})),...positioned];
    }else if(['title','section'].includes(layout)) {
        // The title renderer does not render flow blocks. Keep them as overlays.
        slide.overlays=[...flow.map((b,i)=>({...b,frame:b.frame||measurements[b.id]||{x:80,y:360+i*110,w:1120,h:100}})),...positioned];
    }else {
        if(previous==='canvas') { flow.push(...positioned.filter(b=>b.type!=='group').sort((a,b)=>(a.frame?.y||0)-(b.frame?.y||0)||(a.frame?.x||0)-(b.frame?.x||0)).map(b=>{const c=clone(b);delete c.frame;return c;}));positioned=positioned.filter(b=>b.type==='group'); }
