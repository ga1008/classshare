...
export function openActions(store,api,onError) {
    ...
    let steps=clone(block.actions||[]),once=!!block.once,active=true;const serial=store.serial;
    dialog('点击动作',({body,foot,close})=>{
        ...
        const render=()=>{
            list.replaceChildren();steps.forEach((step,index)=>{
                ...
                if(['move','moveTo','show','hide'].includes(step.do)){row.append(field('动画时长',step.ms??400,value=>step.ms=value,{type:'number',min:0,max:5000,step:50,format:seconds}),field('速度曲线',step.ease||'inout',value=>step.ease=value,{choices:{linear:'匀速',in:'渐入',out:'渐出',inout:'平滑'},visual:'ease'}));}
        ...
        foot.append(button('取消',close),button('应用动作',async()=>{
            try{if(serial!==store.serial)throw new Error('正文已变化，请重新打开动作设置。');const snapshot=JSON.stringify({steps,once}),candidate=clone(store.model),target=locate(candidate,id).block;target.actions=steps;target.once=once;
                ...
                if(!active)return;if(serial!==store.serial||snapshot!==JSON.stringify({steps,once}))throw new Error('预检期间内容已变化，请再次应用。');
