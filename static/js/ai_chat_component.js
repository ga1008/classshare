 ...
 *
 * 依赖:
 * - marked + MarkdownRuntime (来自 partials/markdown_assets.html)
...
function normalizeAIChatMarkdown(content) {
    // Markdown whitespace and delimiters carry meaning, including inside code
    // and link destinations. Let the parser handle the original syntax; regex
    // "repairs" can silently change both rendered examples and copied code.
    return String(content ?? '').replace(/\r\n?/g, '\n');
}

window.normalizeAIChatMarkdown = normalizeAIChatMarkdown;
// The classroom loads this file via import(), other pages use a classic script.
window.renderAIChatMarkdown = renderAIChatMarkdown;
