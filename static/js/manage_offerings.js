...
function applyQueryDefaults() {
    ...
    const offeringId = Number(params.get('offering_id') || 0);
    if (offeringId && offeringMap.has(offeringId)) {
        // 课堂运行页「编辑配置」深链：直接载入该课堂进入编辑态。
        populateForm(offeringMap.get(offeringId));
        return;
    }
