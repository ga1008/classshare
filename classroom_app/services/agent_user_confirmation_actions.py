...
from . import agent_route_confirmation_service as routes
...
_HANDLERS = {action: module for module in (grades, signatures, teaching, routes) for action in module.ACTION_DEFINITIONS}
...
def _extra(handler, app):
    # Only the generic route confirmation performs an in-process HTTP call and
    # therefore needs the mounted application; domain modules keep their contract.
    return {"app": app} if handler is routes else {}


def prepare_user_confirmation(conn, *, action: str, params: dict, user: dict, app=None) -> dict:
    handler = _handler(action, user)
    return handler.prepare_user_confirmation(conn, action=action, params=params, user=user, **_extra(handler, app))


def dispatch_user_confirmation(conn, *, user: dict, source_session_id: str, task_id: int,
                               operation_id: str, action: str, params: dict, confirmation_inputs: dict, app=None) -> dict:
    handler = _handler(action, user)
    return handler.dispatch_user_confirmation(conn, user=user, source_session_id=source_session_id,
        task_id=task_id, operation_id=operation_id, action=action, params=params,
        confirmation_inputs=confirmation_inputs, **_extra(handler, app))
