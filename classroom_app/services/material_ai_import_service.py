...
MATERIAL_AI_IMPORT_GROUPS: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "types": [
            ...
            {
                "key": ORDINARY_GRADE_RECORD_TYPE,
...
async def parse_material_document(
    *,
    ...
    if type_meta["group_key"] == "final_material" and type_meta["key"] == ORDINARY_GRADE_RECORD_TYPE:
        parsed = await asyncio.to_thread(parse_ordinary_grade_record_file, file_path, original_name)
        content_quality = _assess_text_quality(parsed.content_markdown, method="ordinary_grade_excel_formula_parser")
        payload = {
            ...
            "extraction": {
                "method": "ordinary_grade_excel_formula_parser",
        ...
        return MaterialParseResult(
            ...
            warnings=parsed.warnings,
            export_payload=parsed.export_payload,
            raw_ai_result={"local_parser": ORDINARY_GRADE_RECORD_TYPE, "formula_count": parsed.formula_count},
