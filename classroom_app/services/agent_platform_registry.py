...
READ_OPERATIONS = (
    ReadOperation("submission.review_files", "assignments", "教师分页查看答卷附件标识，供授权读取或下载", "/api/submissions/{submission_id}/review-files",
                  "classroom_app.routers.homework_parts.grading.list_submission_review_files", roles=("teacher",),
                  parameters={**_id("submission_id"), "limit": {"in": "query", "type": "integer", "minimum": 1, "maximum": 50},
                              "offset": {"in": "query", "type": "integer", "minimum": 0, "maximum": 10000}}),
    ReadOperation("exam.catalog", "assignments", "教师分页检索有权使用的试卷目录", "/api/exam-papers/review-catalog",
                  "classroom_app.routers.homework_parts.exam_papers.review_exam_catalog", roles=("teacher",),
                  parameters={"limit": {"in": "query", "type": "integer", "minimum": 1, "maximum": 50},
                              "offset": {"in": "query", "type": "integer", "minimum": 0, "maximum": 10000},
                              "q": {"in": "query", "type": "string", "maxLength": 200}}),
    ReadOperation("exam.review", "assignments", "教师读取可用试卷内容、评分标准和编辑版本", "/api/exam-papers/{paper_id}/review",
                  "classroom_app.routers.homework_parts.exam_papers.review_exam_paper", roles=("teacher",),
                  parameters={"paper_id": {"in": "path", "type": "string", "maxLength": 80, "required": True}}),
    ReadOperation("submission.review", "assignments", "教师读取完整答卷、评分标准和当前核对版本", "/api/submissions/{submission_id}/review",
                  "classroom_app.routers.homework_parts.grading.get_submission_review", roles=("teacher",), parameters=_id("submission_id")),
    ReadOperation("assignment.review_submissions", "assignments", "教师分页查看作业答卷目录", "/api/assignments/{assignment_id}/review-submissions",
                  "classroom_app.routers.homework_parts.grading.list_review_submissions", roles=("teacher",),
                  parameters={"assignment_id": {"in": "path", "type": "string", "maxLength": 80, "required": True},
                              "limit": {"in": "query", "type": "integer", "minimum": 1, "maximum": 50},
                              "offset": {"in": "query", "type": "integer", "minimum": 0, "maximum": 10000}}),
